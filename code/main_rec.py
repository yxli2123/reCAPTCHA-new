...
def main():
    ...
    if args.mode == 'train':
        ...
        #            DATA          #
        ############################
        train_set = LineCAPTCHA_rec(args.data_dir, split='train')
        valid_set = LineCAPTCHA_rec(args.data_dir, split='valid')
        ...
        ############################
        # Build Models
        model = resnet(pretrained=False, num_classes=args.num_cls)
        ...
        for epoch in range(n_epochs):
            for t, batch in enumerate(train_loader):
                ...
                y_gt = batch['label'].reshape(B*N).squeeze()   # (B*N, )
                ...
                if train_iter % args.print_interval == 0:
                    ...
                    writer.add_images('train/images', x, train_iter)
        ...
        test_set = LineCAPTCHA_rec(args.data_dir, split='test')
...
def test(dataloader, model, device, criterion, args):
    ...
    for t, batch in enumerate(tqdm(dataloader)):
        ...
        x = x.reshape(B * N, C, H, W)     # (N, C, H, W)

        y_pr = model(x)                   # (N, num_cls)
        y_gt = batch['label'].reshape(B*N).squeeze()   # (N, )
        
        y_logist_list.append(y_pr.cpu())
        ...
        # Classify
        y_pr = torch.argmax(y_pr, dim=1)  # (N, )
    ...
    y_logist = torch.cat(y_logist_list, dim=0)  # (N_all, num_cls)
    loss = torch.tensor(loss_list).mean().item()

    metric = accuracy(y_gt, y_pr)
    metric['loss'] = loss
